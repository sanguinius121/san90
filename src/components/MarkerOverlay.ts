...
import { spectrumAmplitudeFraction } from '../rendering/amplitudeScale'
...
export function MarkerOverlay(ctx: CanvasRenderingContext2D, area: AxisGeometry, marker: Marker | null, minDbm:number,maxDbm:number,startHz:number,stopHz:number) {
  ...
  const y = area.bottom - spectrumAmplitudeFraction(marker.amplitudeDbm, { minDbm, maxDbm }) * (area.bottom - area.top)

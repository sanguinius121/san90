...
int SWP_Meas_OBW()
{
	...
	SWP_AutoSet(&Device, SWPOBWMeas, &SWP_ProfileIn, &ProfileSetOut, &TraceInfo, IfDoConfig); // This function gives the recommended device configuration based on the application target for the spectrum analyzer's sweep mode (SWPMode).
	...
	float OccupiedRatio = 0.99;                // Bandwidth occupying 99% of the total signal power.
	TraceAnalysisResult_OBW_TypeDef OBWResult; // Occupied bandwidth information under the current configuration.

	DSP_TraceAnalysis_OBW(Frequency.data(), PowerSpec_dBm.data(), TraceInfo.FullsweepTracePoints, OccupiedRatio, &OBWResult);

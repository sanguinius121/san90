...
int SWP_Meas_ChannelPower()
{
	...
	SWP_AutoSet(&Device, SWPChannelPowerMeas, &SWP_ProfileIn, &ProfileSetOut, &TraceInfo, IfDoConfig); // This function gives the recommended device configuration based on the application target for the spectrum analyzer's sweep mode (SWPMode).
	...
	DSP_ChannelPowerInfo_TypeDef ChannelPowerResult; // Initialize channel power structure.

	Status = DSP_TraceAnalysis_ChannelPower(Frequency.data(), PowerSpec_dBm.data(), TraceInfo.FullsweepTracePoints, CenterFrequency, AnalysisSpan, SWP_ProfileOut.RBW_Hz, &ChannelPowerResult); // Analyze the channel power of the trace.

...
```json
{
  ...
  "power_min_dbm": -120.0,
  "power_max_dbm": -20.0,
  "power_range_db": 100.0,
  "power_range_generation": 4,
...
Clipping ratios use `<= power_min_dbm` and `>= power_max_dbm`. The image
statistics never alter the committed mapping. `power_range_generation` is a
runtime correlation token and may restart at zero after backend restart; it is
not an analyzer configuration generation. These fields extend protocol-v1 JSON
compatibly; multipart framing remains exactly two parts.

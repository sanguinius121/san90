...
| VBW | fixed `ratio-0.1`; read-only `vbw_hz` | `RTA_Profile_TypeDef.VBWMode`, enum/`int`; `VBW_Hz`, `double` Hz | Native mappings are Manual 0, RBW 1, 0.1× RBW 2, 0.01× RBW 3, 10× RBW/bypass 4. The application always configures mode 2 and exposes no writable VBW control. | 0.1× actual RBW | Both mode and Hz from returned `ProfileOut`; the UI polls and displays `VBW_Hz` rather than synthesizing it | Fixed 0.1× avoids user-selected trace-rate and AI-image changes. |

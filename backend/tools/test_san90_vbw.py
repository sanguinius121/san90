"""Short, non-destructive SAN-90 fixed-VBW hardware diagnostic."""
...
from backend.analyzer.vbw import VBW_FIXED_MODE
...
def main() -> int:
    ...
    try:
        ...
        source.start()
        run_case(source, f"auto_rbw_{VBW_FIXED_MODE}", conservative, args.sample_seconds)
        for rbw_request in (50_000.0, 300_000.0):
            request = replace(conservative, rbw_mode="manual", rbw_hz=rbw_request)
            run_case(source, f"rbw_{rbw_request:g}_{VBW_FIXED_MODE}", request, args.sample_seconds)
        source.apply_settings(conservative)
    finally:
        source.disconnect()
    return 0
